...
#define TINY_GSM_CLIENT_CONNECT_OVERRIDES                                                                                                                                                              \
    int connect(IPAddress ip, uint16_t port, int timeout_s) { return connect(TinyGsmStringFromIp(ip).c_str(), port, timeout_s); }                                                                      \
    int connect(const char *host, uint16_t port) override { return connect(host, port, 75); }                                                                                                          \
    int connect(IPAddress ip, uint16_t port) override { return connect(ip, port, 75); }
...
template <class modemType, uint8_t muxCount> class TinyGsmTCP {
  public:
    /*
     * Basic functions
     */
...
#error Modem client has been incorrectly created
    ...
    inline void moveCharFromStreamToFifo(uint8_t mux)
    {
        if (!thisModem().sockets[mux])
            return;
        uint32_t startMillis = millis();
        while (!thisModem().stream.available() && (millis() - startMillis < thisModem().sockets[mux]->_timeout)) {
            TINY_GSM_YIELD();
        }
